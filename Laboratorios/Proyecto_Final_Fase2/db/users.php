...
function get_users() {
    $conn = connect_db();
    if (!$conn) return ["status" => "error", "message" => "Error connecting to database"];
    ...
    try {
        ...
        return ["status" => "error", "message" => "Error fetching users: " . $e->getMessage()];
    }
}
...
function login($username, $password) {
    ...
    try {
        ...
        return ["status" => "error", "message" => "Error during login: " . $e->getMessage()];
    }
}

/**
 * Verifica si el usuario tiene el nivel de permiso requerido
 * @param int $requiredLevel Nivel requerido (1: administrador, 2: contador, 3: vendedor, 4: invitado)
...
function add_user($user_data) {
    ...
    try {
        ...
        return ["status" => "error", "message" => "Error adding user: " . $e->getMessage()];
    }
}
...
function update_user($user_data) {
    ...
    try {
        ...
        return ["status" => "error", "message" => "Error updating user: " . $e->getMessage()];
    }
}
...
function delete_user($id) {
    ...
    try {
        ...
        if ($count <= 1) {
            return ["status" => "error", "message" => "Cannot delete the last user"];
        }
        ...
        return ["status" => "error", "message" => "Error deleting user: " . $e->getMessage()];
    }
}
...
function get_session_info() {
    if (!isset($_SESSION['user_id'])) {
        return ["status" => "error", "code" => "not_authenticated", "message" => "No active session"];
    }
